...
tr="$(cat /etc/xray/grpc/trojangrpc.json | grep port | sed 's/"//g' | sed 's/port//g' | sed 's/://g' | sed 's/,//g' | sed 's/ //g')"
until [[ $user =~ ^[a-zA-Z0-9_]+$ && ${user_EXISTS} == '0' ]]; do
		read -rp "Username : " -e user
		user_EXISTS=$(grep -w $user /etc/xray/grpc/akuntrgrpc.conf | wc -l)
...
},{"password": "'""$uuid""'"' /etc/xray/grpc/trojangrpc.json
echo -e "### $user $exp" >> /etc/xray/grpc/akuntrgrpc.conf
