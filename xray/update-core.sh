...
get_latest_version() {
  ...
  if ! curl -x "${PROXY}" -sS -H "Accept: application/vnd.github.v3+json" -o "$tmp_file" 'https://api.github.com/repos/dharak36/Xray-core/releases'; then
    "rm" "$tmp_file"
    echo 'error: Failed to get release list, please check your network.'
    exit 1
  fi
  ...
  do
    ...
    grep -q "https://github.com/presult77/script/raw/main/xray.linux.64bit.zip"
  done
  "rm" "$tmp_file"
...
download_xray() {
  DOWNLOAD_LINK="https://github.com/presult77/script/raw/main/xray.linux.64bit.zip"
...
main() {
  ...
  ZIP_FILE="${TMP_DIRECTORY}/xray.linux.64bit"
