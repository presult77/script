#!/usr/bin/env bash
# Cek login Xray VMess tanpa hapus log & tanpa sleep; cepat dan robust.

set -Eeuo pipefail
IFS=$'\n\t'

# === Konfigurasi ===
ACCESS_LOG="/var/log/xray/access-vmess.log"
VMESS_JSON="/etc/xray/vmess.json"
SERVICE_NAME="vmess"        # ganti bila servicenya berbeda
...
[[ -r "$VMESS_JSON" ]] || { echo "Gagal membaca $VMESS_JSON"; exit 1; }

# Ambil daftar user dari vmess.json
# Konvensi: baris yang dimulai '###' diikuti username sebagai field ke-2
# Contoh: "### user@example.com"
mapfile -t USERS < <(grep -E '^\s*###' "$VMESS_JSON" | awk '{print $2}' | sort -u)
if [[ ${#USERS[@]} -eq 0 ]]; then
  echo "Tidak ditemukan user dari $VMESS_JSON (baris '### <user>')."
fi
...
awk -v start_epoch="$start_epoch" -v users="$(printf '%s\n' "${USERS[@]-}" | tr '\n' ' ')" '
...
BEGIN{
  ...
  print "-------------------------------";
  print "-----=[ Xray VMess Login ]=-----";
  print "-------------------------------";
}
# Hanya baris bertimestamp + mengandung "email"
...
END{
  ...
  # IP yang terlihat tapi tidak match user known mana pun
  print "other";
  idx=0;
